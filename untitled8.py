...
if not api_key:
    ...
    if st.button("レシピを提案して！"):
        if not ingredients or not mood:
         ...
         # 🖼️ Pixabay画像を表示（AIで最適な検索キーワードを生成）
try:
    recipe_name = recipe.splitlines()[0].replace("1. ", "").strip()

    # ① AIに「この料理の検索キーワード」を作らせる
    keyword_prompt = f"""
    この料理名『{recipe_name}』の見た目の特徴を
    画像検索用のキーワードとして10〜15語で作ってください。
    ...
    keyword_res = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[
            {"role": "system", "content": "あなたは料理の見た目を説明する専門家です。"},
            {"role": "user", "content": keyword_prompt}
        ]
    )

    keywords = keyword_res.choices[0].message.content
    st.write("🔍 画像検索キーワード:", keywords)

    # ② Pixabay API
    PIXABAY_KEY = st.secrets["PIXABAY_API_KEY"]

    # ③ AI が作ったキーワードで検索
    url = f"https://pixabay.com/api/?key={PIXABAY_KEY}&q={keywords}&image_type=photo&orientation=horizontal"
    ...
    if "hits" not in data or len(data["hits"]) == 0:
        st.warning("⚠️ 画像が見つかりませんでした。検索ワードを調整してください。")
    else:
